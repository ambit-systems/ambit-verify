...
are retained from genesis through the attested head, not proved to be the latest
head. Observations cannot be omitted from that authenticated prefix.
"""
...
def verify_behavioural_source(
    ...
    *,
    ...
    for index, record in enumerate(records):
        ...
        if checked.error is not None or checked.record is None:
            raise BehaviouralVerificationError(checked.error or "source: invalid record")
        previous = checked.record_hash
        verified_records.append(dict(checked.record))
